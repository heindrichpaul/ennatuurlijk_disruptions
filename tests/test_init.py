...
async def test_migrate_v1_global_entry(hass):
    ...
    hass.config_entries.async_update_entry.assert_any_call(
        entry, unique_id="ennatuurlijk_global"
    )
    hass.config_entries.async_update_entry.assert_any_call(
        entry,
...
async def test_migrate_v1_subentry_with_existing_global(hass):
    global_entry = SimpleNamespace(
        unique_id="ennatuurlijk_global", version=2, data={}, options={}
    )
    ...
    hass.config_entries.async_update_entry.assert_any_call(
        entry,
        title=" - 1234AB",
        data={"name": "Ennatuurlijk Disruptions", "town": "", "postal_code": "1234AB"},
        options={},
        version=2,
    )
...
async def test_migrate_v1_subentry_migrates_v1_global_first(hass):
    ...
    hass.config_entries.async_entries.side_effect = [
        [],
        [v1_global],
        [SimpleNamespace(unique_id="ennatuurlijk_global")],
    ]
    with patch.object(
        integration, "async_migrate_entry", wraps=integration.async_migrate_entry
    ) as migrate_patch:
        result = await integration.async_migrate_entry(hass, entry)
        assert result is True
    ...
    hass.config_entries.async_update_entry.assert_any_call(
        entry,
        title=" - 5678CD",
        data={"name": "Ennatuurlijk Disruptions", "town": "", "postal_code": "5678CD"},
        options={},
        version=2,
    )
...
async def test_migrate_v1_subentry_creates_global_if_missing(hass):
    ...
    hass.config_entries.async_update_entry.assert_any_call(
        entry,
        title=" - 9999ZZ",
        data={"name": "Ennatuurlijk Disruptions", "town": "", "postal_code": "9999ZZ"},
        options={"days_to_keep_solved": 5, "update_interval": 42},
        version=2,
    )
