...
@pytest.mark.asyncio
async def test_solved_only_event_single_day_and_hashtag(hass, mock_aiohttp_session, mockEntry):
    ...
    assert any("Status: #solved" in ev.description or "Status: #completed" in ev.description for ev in events)


@pytest.mark.asyncio
async def test_current_then_solved_is_single_event(hass, mock_aiohttp_session, mockEntry):
